...
int main()
{
	...
	FILE* fp4 = fopen("train-labels.idx1-ubyte", "rb");
	for (int l = 0; l < 8; l++)
		fgetc(fp4);
	...
	FILE *fp5 = fopen("train_labels.txt", "w");
	FILE *cch = fopen("cch.txt", "w");
	...
	//	int is1or7 = ( (int)labels[a] == 1 ) || ( (int)labels[a] == 7 );

	//	if(count > 60 || (is1or7 && count > 30))
		if(finished_flag)
		{
			fprintf(fp5,"%d\n",(int)labels[a]); 	

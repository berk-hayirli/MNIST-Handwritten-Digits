...
int main()
{
	...
	Mat labelsMat(TRAIN_DATA_SIZE, 1, CV_32S, labels);

	Ptr<NormalBayesClassifier> bayes = NormalBayesClassifier::create();
	bayes->train(trainingDataMat, ml::ROW_SAMPLE, labelsMat);
	...
	for (int i = 0; i < TEST_DATA_SIZE; ++i) 
		...
		float pred = bayes->predict(testDataMat);
